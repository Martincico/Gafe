...
namespace GAFE
{
    class DocRegRequisiciones
    {
        ...
        public SqlDataAdapter GetDoc_PorDocOrigenSql()
        {
            //Al eliminar una Orden de salida, primero se eliminar la orden de entrara, para eso 
            //Entrará acá para saber su Documento relacionado. Elimnar este Registro (Orden de salida) y luego el
            //Seleccionado (Orden Entrada)
            SqlDataAdapter dt = null;
            string Sql = " SELECT RM.idMov,RM.Documento,RM.Serie,RM.NumDoc,RM.ClaveAlmacen," +
                         "       RM.FechaExpedicion,RM.ClaveImpuesto,RM.Impuesto,RM.Descuento, RM.SubTotal," +
                         "       RM.Total,RM.Observaciones,RM.Estatus,RM.Autorizado," +
                         ...
                         " FROM DocCab AS RM " +
                         " INNER JOIN dbo.Inv_CatAlmacenes AS Alm ON RM.ClaveAlmacen = Alm.ClaveAlmacen " +
                         " WHERE RM.DocOrigen = @DocOrigen AND RM.Estatus = 1 ";
            dt = db.SelectDA(Sql, ArrParametros);
            return dt;
        }
